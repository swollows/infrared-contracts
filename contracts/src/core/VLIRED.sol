...
contract VLIRED is ReentrancyGuard, Ownable {
    ...
    /**
     * @param  _ired  address  IRED token address
     */
    constructor(address _ired) Ownable(msg.sender) {
    ...
    function lock(address account, uint256 amount) external nonReentrant {
        ...
        IRED.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function _processExpiredLocks(address account, bool relock, address withdrawTo) internal {
        ...
        if (relock) {
            ...
            IRED.safeTransfer(withdrawTo, locked);
        }
    }
